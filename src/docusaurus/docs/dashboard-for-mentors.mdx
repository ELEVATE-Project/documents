...
You can view the mentoring session details, such as the session title, who created the session, how many mentees attended the session, the date on which the session was conducted, the session type, (such as public or private), whether the session was conducted (yes or no),
...
3. **Select dropdown list**: Choose an appropriate value from the select dropdown list to display the result that you require.
